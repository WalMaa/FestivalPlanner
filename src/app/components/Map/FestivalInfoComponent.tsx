...
const FestivalInfoComponent = ({ city, festival, setExpandedLocation }: { city: string, festival?:string, setExpandedLocation: React.Dispatch<React.SetStateAction<string | null>> }) => {
    ...
    return ReactDom.createPortal(
        <>
            <div className="fixed top-0 left-0 w-full h-full z-10 backdrop-blur-sm">
                <div ref={modalRef} className="absolute top-1/4 right-1/2 max-w-sm sm:max-w-fit translate-x-1/2 bg-white shadow-md rounded-lg p-2 z-20 ">
                    <div className="flex flex-col">
                        ...
                        <div>
                            {festivalsAtLocation.map((festival: Festival) => (
                                <div key={festival.id} className={`${selectedFestival === festival.id ? 'block' : 'hidden'} flex-col bg-gray shadow-md mt-2 rounded-lg`}>
