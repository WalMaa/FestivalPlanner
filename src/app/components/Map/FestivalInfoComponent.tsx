    ...
    return ReactDom.createPortal(
        <>
            <div className="fixed top-0 left-0 w-full h-full z-10 backdrop-blur-sm">
                <div ref={modalRef} className="absolute top-1/4 right-1/2 max-w-sm sm:max-w-fit translate-x-1/2 backdrop-blur bg-opacity-10 bg-gray shadow-md rounded-lg p-2 z-20 ">
                    <div className="flex flex-col">
                        <div className="flex justify-center items-center">
                            ...
                            <button onClick={() => closeLocation()} aria-label='Close festival menu' className="absolute right-2">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className=" w-8 h-8 hover:scale-110 transition-transform duration-300 active:scale-90 hover:stroke-2">
                                    <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
                                </svg>
                            </button>
                        </div>
                        <div className="flex justify-evenly">
                            {festivalsAtLocation.map((festival: { id: string; name: string; }) => (
                                <div key={festival.id}>
                                 ...
                                 ${selectedFestival === festival.id ? 'bg-gradient-to-b from-white to-gray rounded-b-none scale-105' : ''}`}
